...
async function syncToDB() {
  ...
  for (const discSlug of disciplines) {
    ...
    for (const topicSlug of topics) {
      ...
      if (errTop) {
        stats.topics.errors++;
        errorLogs.push(`[TOPIC SYNC ERROR] ${topicSlug}: ${errTop.message}`);
        continue;
      }
      ...
      if (topicData.modules) {
        for (const localMod of topicData.modules) {
          ...
          const { data: dbMod, error: errMod } = await supabase
            .from('modules')
            .upsert({ slug: localMod.id, title: localMod.title, topic_id: dbTop.id }, { onConflict: 'slug' })
            .select().single();
            
          if (errMod) {
            stats.modules.errors++;
            errorLogs.push(`[MODULE SYNC ERROR] ${localMod.id}: ${errMod.message}`);
            continue;
          }
      ...
      // Sync Questions -> Linked to Topic directly
      ...
      if (fs.existsSync(questionsJsonPath)) {
        ...
        for (const q of questionsData) {
          ...
          if (errQ) {
            stats.questions.errors++;
            errorLogs.push(`[QUESTION SYNC ERROR] ${q.id}: ${errQ.message}`);
          } else {
  ...
  console.log(`${stats.topics.found} topics`);
  console.log(`${stats.modules.found} modules`);
