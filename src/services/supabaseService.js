...
export async function fetchFlashcards(disciplineId) {
  ...
  // Get local questions to derive flashcards
  const questions = content.getAllQuestionsByDiscipline(disciplineId);
  
  // Track IDs of real flashcards generated by 'addWrongQuestionToReview' (they use prefix 'review_')
  // We extract the original question ID to avoid duplicating them with a 'derived_' flashcard.
...
export async function fetchQuestions(disciplineId, topicId = null) {
  const all = content.getAllQuestionsByDiscipline(disciplineId);
  if (topicId) {
    return Promise.resolve(all.filter(q => q.topic_id === topicId));
  }
