...
old_hf = r'''const handleFlashcardEval = async \(evaluation\) => \{.*?\} catch \(err\) \{.*?\}\n  \};'''
new_hf = r'''const handleFlashcardEval = async (evaluation) => {
...
code = re.sub(old_hf, new_hf, code, flags=re.DOTALL)

# 4. handleQuestionAnswer
old_hq = r'''const handleQuestionAnswer = async \(option\) => \{.*?\} catch \(err\) \{\n\s*console.error\("Error saving attempt:", err\);\n\s*\}\n\s*\}\n\s*\};'''
new_hq = r'''const handleQuestionAnswer = async (option) => {
...
code = re.sub(old_hq, new_hq, code, flags=re.DOTALL)

# 5. startStudying
old_ss = r'''const startStudying = async \(disc\) => \{.*?setLoading\(false\);\n  \};'''
new_ss = r'''const startStudying = async (disc, topicId = null) => {
    setLoading(true);
    setErrorMsg("");
...
code = re.sub(old_ss, new_ss, code, flags=re.DOTALL)

# 6. onClick for disciplines
code = code.replace(
    'onClick={() => startStudying(d)}',
...
topic_selection_code = r'''
  ...
  if (step === "studying") {'''
code = code.replace('  if (step === "studying") {', topic_selection_code)

# 8. Finished Step
old_fin = r'''if \(step === "finished"\) \{.*?return null;\n\}'''
new_fin = r'''if (step === "finished") {
    if (mode !== "flashcards") {
...
code = re.sub(old_fin, new_fin, code, flags=re.DOTALL)
